...
help:
	...
	@echo "  release-linux          Linux 发布包（AppImage + deb + CLI tar.gz），在 Linux 上运行"
	@echo "  release-windows        Windows 发布包（NSIS .exe + 绿色版 zip + CLI zip），在 Windows 上运行"
	@echo "  release-macos          macOS 发布包（Tauri .dmg + CLI tar.gz），在 macOS 上运行"
