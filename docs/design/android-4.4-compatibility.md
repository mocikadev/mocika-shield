...
GUI 和 CLI 后续读取原 APK 的 `minSdkVersion` 与 `lib/<ABI>/`，自动给出建议，由用户最终确认。
...
5. **真实工控板回归**：使用同一次加固产物，在用户 Android 4.4.2 和 Android 6.0 工控板验证主要业务及硬件交互。
6. **产品接入**：验证稳定后才引入双资源选择、APK 最低版本与 ABI 预检、GUI/CLI 模式选择。
