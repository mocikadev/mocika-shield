...
- 运行环境保护使用 `compatible/strict` 固定枚举；缺省值保持兼容模式，前端不得提交其他策略值
...
加固页和签名页的输出路径均允许在任务开始前修改。任务开始时固化 APK、运行系统兼容性、运行环境保护策略、证书、签名选项和输出路径；执行中以及完成、失败后的结果展示阶段均不可修改，只有点击“继续加固”或“继续签名”重置任务后才重新开放选择。运行环境保护默认使用 `compatible`；`strict` 命中高置信 Root 或注入信号时拒绝启动，界面必须提示 Root、模拟器和工控设备的误判风险。签名完成后，页面不再展示“开始签名”主按钮，只保留“继续签名”入口，避免完成态出现两个含义接近的操作。
