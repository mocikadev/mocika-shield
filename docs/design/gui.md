...
- 主工作区展示 APK、预检结论、风险摘要、输出文件名和保存目录。预检详情按需展开，包含 SDK 范围、单独 split APK 阻断、`extractNativeLibs` 策略和已有 HTTP 兼容信号；正常情况下不堆叠所有通过项。
...
### 4.3 应用使用情况分享（Beta.6 起）

加固页在开始按钮附近只增加一行“分享加固使用情况”，签名页对应为“分享签名使用情况”。沿用现有复选框、字号、焦点及禁用状态，不新增卡片、解释行、查看详情入口或确认弹窗，不挤占阶段进度。详细数据说明放设置页，区分匿名统计、逐次确认的错误报告和应用分享。

两页按可靠解析的包名共用 `config.toml` 中的选择：新包名默认选中，取消对该包名持久有效；换版本、路径或名称不恢复选中。识别中或无法识别时未选中并禁用，不阻止原操作。快速换包不能应用旧响应；任务开始及结果展示阶段保持不可变选择。另一页面取消后，正在执行任务的显示不改变，但后端停止该包名后续发送并清理未发送记录。

前端不生成应用分享正文，仅传后端检查引用及布尔选择。独立签名和加固成功分别记录，自动签名不另生成独立签名记录；保存或上报失败不影响加固和签名结果。完整边界见[应用使用情况分享](application-usage-sharing.md)。本节不表示线上 Beta.5 已具备此能力。
