...
- 同一样本各采集五次冷启动与启动完成后即时 TOTAL PSS，中位数如下。采样来自单台设备，受调试探针、系统状态和即时采样时序影响，只作为优化前基线。

| 形态 | 冷启动中位数 | 即时 TOTAL PSS 中位数 |
|------|--------------|-----------------------|
| 未加固 | 250 毫秒 | 13187 KB |
| 认证文件路径 | 320 毫秒 | 19079 KB |
| 内存路径原型 | 382 毫秒 | 43595 KB |

内存原型相对认证文件路径的冷启动约增加 19%，即时 TOTAL PSS 约增加 129%。当前差距超过生产候选可接受范围，必须先确认 ByteBuffer、解密结果、代理及原始 APK 路径是否形成重复长期持有，再扩大设备矩阵。`memory_dex` 继续保持关闭；自动崩溃回退、代码 split 加密和内存占用收敛仍是进入开发资源前的阻塞项。
