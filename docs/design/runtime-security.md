...
首个实验路径和生产候选都只考虑 API 29+。实验实现提前到 `1.3.0-alpha.2`，默认关闭且不进入正式 GUI；API 26～28 在专项证据充分前继续走文件路径。实验未达到门槛时不得阻塞 `1.3.0`，也不得以未验证状态进入正式资源。
...
发布门槛：候选版本的设备矩阵、用户验证和三平台发布构建全部通过；存在未完成的 Android 4.4 真机阻塞项时不得仅为推进版本号发布。

### `1.3.0-alpha.1`：认证缓存与资源能力
...
### `1.3.0-alpha.2`：API 29 以上内存 DEX 实验
