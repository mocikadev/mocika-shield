...
允许缓存目录额外包含真实的 `oat/` 子目录，供 ART 保存优化产物；该目录不计入载荷文件数量，也不参与 DEX 根摘要。仍拒绝同名普通文件、顶层符号链接、未知目录和额外 DEX。优化产物由系统管理，允许其存在不代表本工具对其内容提供完整性认证。

Issue #117 暴露了旧版按目录项总数校验的缺陷：系统生成 `oat/` 后，未改变的 DEX 也会被误判失效，触发递归清理和重新解密。修复保持原缓存身份协议不变，加固工具更新后需重新加固 APK 才能使用新壳逻辑。缓存递归清理不得跟随符号链接删除目录外文件。

回归覆盖：只读缓存首次验证通过、加入模拟优化产物后连续验证仍通过、存在优化目录时篡改或缺失 DEX 仍被拒绝、额外 DEX 被拒绝，以及清理顶层和优化目录内部链接时保留外部文件。本地 JVM 测试验证缓存逻辑，不代替 Android 设备上的连续启动与 dex2oat 日志验证。
...
3. 自动重启回退使用持久化状态记录本次启动路径；发生异常后只允许在独立进程的下一次启动切换到文件路径，状态必须认证且不能绕过缓存安全检查。
...
- 每个代码 split 的加密、协议映射和厂商 ClassLoader
- 崩溃后的认证回退状态、独立进程重启和更广泛版本迁移
- 更广泛厂商真机的性能和内存矩阵
...
正式接入仍需把原工厂类名以独立元数据保存，区分“原应用未声明工厂”、AndroidX 工厂、自定义工厂和错误地指回壳工厂的递归配置；委托创建失败必须失败关闭，不能静默退回默认工厂。在这些边界完成前，不开放 `memory_dex` 能力字段。

#### 延迟内存加载代理

2026-07-30 在隔离探针中验证了不削弱签名绑定的延迟初始化方案：

1. `AppComponentFactory.instantiateClassLoader()` 只创建并返回身份稳定的代理加载器，不读取、解密或定义业务类。
2. 默认加载器只负责定义壳工厂和壳 Application；代理未初始化时拒绝业务类加载，避免业务类被错误的加载器提前定义。
3. 壳 Application 在 `attachBaseContext()` 获得 `Context` 后，执行现有签名绑定解密流程，创建真实 `InMemoryDexClassLoader` 并一次性挂接到代理。
4. 真实 Application 和其余业务组件随后都通过同一代理身份加载；重复初始化保持同一加载器，并发首次加载不同业务类也必须稳定。

该方案消除了“工厂回调阶段没有 `Context`”这一架构阻塞，不需要提前读取证书或降低 DEXB v5 的签名绑定强度。初始化、解密或原工厂创建任一步失败都必须在业务类定义前失败关闭；同一进程一旦开始定义业务类，不允许再切回文件加载路径。

API 29 验证还发现，Receiver 等组件回调可能落到另一个壳工厂实例。代理、初始化状态和原工厂委托因此必须由应用进程级状态统一持有，而不能依赖某个工厂对象的实例字段；静态状态只在当前应用进程内生效，远程进程会独立初始化。正式实现应把该状态收敛在单一运行时协调单元中，避免扩散为通用全局状态。
...
Android 9（API 28）不进入 API 29 以上内存 DEX 承诺范围。现有文件加载资源已在 API 28 ARM64 模拟器使用同一 ARouter 样本复验清除数据和同签名覆盖升级，均能跳转真实首页，确认低版本回退未受影响。

#### split、Instrumentation、路径迁移与性能基线

2026-07-30 在 API 35 OnePlus 5 真机补齐首轮生产门禁：

- 同签名外部测试 APK 通过自定义 Instrumentation 获取目标 Context 的稳定代理加载器，加载并启动业务 Activity；业务类由代理内部的真实内存加载器定义，没有泄漏到测试 APK 加载器。
- 使用 Android App Bundle 动态特性生成设备 split 集后，正式 DEXB 基础载荷仍能解密，代理可以访问动态特性类。bundletool 将安装时动态特性代码合入 master split，该类仍可从明文 DEX 直接识别。因此当前只证明 split 安装兼容，代码 split 加密仍是生产阻塞项。
- 真实 ARouter 样本在保持签名和应用数据的情况下完成“认证文件路径 → 内存路径 → 认证文件路径 → 内存路径”连续覆盖安装，并在清除数据和全新安装后通过。该结果证明版本切换不会因旧缓存或旧代理状态阻塞启动，不等同于崩溃后的自动回退已经实现。
- 同一样本使用独立性能变体采集五次冷启动、即时 TOTAL PSS 和统一发送内存整理信号后的稳定 PSS。性能变体不自动跳转 ARouter 首页、不预加载首页类，也不执行阶段观测代码。采样来自单台设备，只作为当前原型基线。

| 形态 | 冷启动中位数 | 即时 TOTAL PSS 中位数 | 整理后 TOTAL PSS 中位数 |
|------|--------------|-----------------------|--------------------------|
| 未加固 | 254 毫秒 | 13195 KB | 13358 KB |
| 认证文件路径 | 337 毫秒 | 19051 KB | 19090 KB |
| 内存路径原型 | 329 毫秒 | 36263 KB | 24511 KB |

最初的 43595 KB 即时 PSS 数据同时受到自动跳转首页、首页类预加载和阶段观测影响，不能与停留在启动页的文件路径直接比较，已废弃。独立诊断变体进一步确认：密文数组、解密后的二维数组、单个 DEX 数组、直接缓冲区数组和首个直接缓冲区在启动后 GC 均不再存活，没有发现 Java 强引用泄漏。

ZIP 条目改为按已声明大小直接读入定长数组后，载荷读取阶段 PSS 从约 9.5 MB 降至 6.0 MB，加载器创建阶段从约 33.5 MB 降至 30.0 MB，减少约 3.5 MB 瞬时峰值。整理后的内存路径仍比认证文件路径高约 5.4 MB（约 28%），与 5.2 MB 业务 DEX 常驻内存的量级接近。继续降低稳定占用需要评估 Native 直接缓冲区接口，可能改变正式 JNI 契约，作为后续独立阶段处理；不得通过启动时强制 GC 换取表面指标。

本轮没有观察到内存路径冷启动回退，但单设备五次中位数不足以证明性能优势。`memory_dex` 继续保持关闭；代码 split 加密、稳定内存扩大验证和认证回退的正式资源集成仍是进入开发资源前的阻塞项。

#### 认证崩溃回退状态机原型

2026-07-30 在隔离探针中实现并通过首轮跨进程回退验证。状态同时绑定当前加密载荷的 SHA-256 身份和进程名摘要，每个应用进程使用独立状态文件，避免 `SharedPreferences` 跨进程缓存与整文件覆盖使一个健康进程被另一个进程误判为崩溃。状态记录由随机软件 HMAC 密钥认证；每个进程的密钥分别使用 Android Keystore 中的 AES-GCM 密钥包装，避免直接用 APK 内可恢复材料生成认证值。启动前的 `pending` 状态同步持久化，成功状态异步提交；若成功提交丢失，下次启动只会保守进入文件路径，不会把一次未确认的内存启动误判为成功。
...
验证使用故障注入使内存路径在业务 Application 启动前崩溃。第二次独立进程启动自动选择文件路径并完成业务启动，第三次启动仍保持粘性文件回退；随后对文件路径再次注入失败，下一次启动能够识别遗留的 `file_pending` 并失败关闭。文件回退只在新的进程启动时发生，不在已经定义业务类的进程中切换加载器。回退 DEX 从已认证的 DEXB 载荷重新解密，以临时文件写入、同步并替换为只读文件，不复用未知来源的旧明文缓存。

主动修改认证记录后，下一次启动能够识别 HMAC 不匹配并在解密和业务类定义前失败关闭。删除包装密钥材料会返回明确的密钥缺失错误，损坏包装密文或丢失对应 Keystore 条目则统一视为解包失败，均不得自动生成新密钥覆盖旧认证状态。主进程和远程进程同时首次启动时，两者均独立进入内存路径、完成状态提交且不出现错误文件回退。

该结果仍不等同于生产能力：状态机目前只在测试夹具中，尚未验证更多进程数量、进程名变更、系统备份恢复和更广泛厂商设备。Root 攻击者能够删除全部应用数据和 Keystore 条目，状态认证不能建立 Root 环境下不可绕过的可信根。正式接入时应把状态所有权收敛到运行时协调单元，并复用正式缓存的目录级原子重建与摘要校验，不复制探针文件实现。
