...
2026-07-30 在隔离探针中实现并通过首轮跨进程回退验证。状态只绑定当前加密载荷的 SHA-256 身份，状态记录由随机软件 HMAC 密钥认证；该密钥使用 Android Keystore 中的 AES-GCM 密钥包装，避免直接用 APK 内可恢复材料生成认证值。启动前的 `pending` 状态同步持久化，成功状态异步提交；若成功提交丢失，下次启动只会保守进入文件路径，不会把一次未确认的内存启动误判为成功。
...
主动修改认证记录后，下一次启动能够识别 HMAC 不匹配并在解密和业务类定义前失败关闭。该结果仍不等同于生产能力：状态机目前只在测试夹具中，尚未验证包装密钥或 Keystore 条目丢失、多进程并发写入以及更广泛厂商设备。Root 攻击者能够删除应用数据和 Keystore 条目，状态认证不能建立 Root 环境下不可绕过的可信根。正式接入时应把状态所有权收敛到运行时协调单元，并复用正式缓存的目录级原子重建与摘要校验，不复制探针文件实现。
