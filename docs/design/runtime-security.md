...
| 分阶段修复检查、认证缓存、接入 Root 策略，最后调研内存 DEX | 每阶段可独立验证、发布和回滚 | 交付周期较长 | 采用 |
...
首个生产候选只考虑 API 29+。API 26～28 在专项证据充分前继续走文件路径；内存 DEX 不作为当前稳定版本的发布阻塞项。
...
验收：普通真机严格模式不误报；可控 Root 环境能够命中；Android 4.4 工控兼容模式默认不被阻断。

### `1.3.0-rc.1`：完整安全回归

范围：只修复 alpha/beta 验证发现的问题，不再扩展安全信号和 UI 范围。完成三平台 GUI 打包、标准/API 19 双资源和全设备矩阵验证后，再决定 `1.3.0` 稳定版。

### `1.4.0-alpha.1`：内存 DEX 实验候选

先提交实验报告和性能数据，再决定是否提供 API 29+ 可选候选路径。未达到多 DEX、ARouter、Android 9、16 KB 和启动性能门槛时，不进入生产资源，也不阻塞 `1.3.0`。
