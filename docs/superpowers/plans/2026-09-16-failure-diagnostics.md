...
在当前工程和功能分支实施，不创建其他工作树。不自动提交、推送、部署、启用接口或发布。完成整个阶段再统一交付。全部注释和文档使用中文。匿名统计受原开关控制；报告逐次确认，绝不上传原始错误或日志。APK 壳、证书数据库格式保持不变。
...
完整本地测试包交付后统一审查提交。线上数据库增量迁移、Worker 部署、报告密钥配置与启用、Beta.5 标签发布需维护者确认；执行顺序见[运维步骤](../../ops/failure-diagnostics.md)。Windows/Linux 原生验收和线上限流验证未执行前不得标为完成。
