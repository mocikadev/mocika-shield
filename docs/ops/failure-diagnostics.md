...
适用目标：`1.4.0-beta.5` 起。2026-09-16 已经维护者授权完成线上增量迁移、Worker 部署和报告入口启用；Beta.5 客户端已发布。执行记录见文末。2026-09-18 上线 Beta.6 分享通道时，核对既有诊断结构完整后补登记了此前直接执行而未记入迁移账本的 `0003_failure_diagnostics.sql`，没有重放旧 ALTER 或重建数据表。
