# Mocika Shield 路线图历史档案

> 本文保存截至 2026-07-31 的旧路线图快照，包含已完成缺陷、版本演进和研究过程，仅供追溯，不再作为当前执行入口。
> 当前计划以 [功能路线图](roadmap.md) 为准；专项技术结论以 `docs/design/` 下对应文档为准。

---

## `1.4.0-alpha.1`：加固前风险检查与操作闭环

已发布三平台桌面 GUI 预发布包。该阶段完成加固页信息重组、常用方案持久化、输出文件名和目录独立配置、二进制 Manifest 只读解析、单 APK 安装形态预检，以及可复制的脱敏诊断摘要。

预检覆盖签名、已有载荷、DEX、ABI、`minSdkVersion`、`extractNativeLibs`、split APK、16 KB 基础信号与 Android 共享库兼容信号；结果按“可继续 / 有风险 / 必须阻止”引导用户。该阶段未改变 DEXB v5、运行时加载或默认安全策略，后续转入 Beta 兼容性收口。
...
| 缓存安全与环境策略 | 区分缓存完整性和明文保密性，提供可选择的运行环境策略 | Native 库名去品牌化、缓存摘要、原子重建、资源能力协议、高版本内存 DEX 实验、Root 兼容/严格策略、Stub DEX 静态治理 | 已完成：`1.3.0` |
| 内存 DEX 生产化 | 在兼容和性能证据充分后减少高版本完整明文 DEX 持久落盘 | API 31+ 生产候选、API 30 以下文件路径、扩大设备验证、发布冻结 | `1.4.0-alpha.1` → `beta.1` → `rc.1` → `1.4.0` |
| AAB 正式支持 | 将已验证的单 base 模块实验推进为 Google Play 可用的正式加固流程 | Play 应用签名绑定、bundletool、ABI 配置 APK、多 DEX、Play 内部测试轨道 | `1.5.0-alpha.1` → `beta.1` → `rc.1` → `1.5.0` |
| 用户诊断与兼容性预检 | 让普通用户在加固前发现风险，遇到问题时能提供有效反馈 | 错误分类、本地加固报告、minSdk/targetSdk、ABI、split APK、已有壳和签名方案预检 | 低风险静态预检可独立插入，完整诊断体系待排期 |
| 自动化与批处理 | 支持更高频、重复性使用场景 | 先稳定非交互 CLI、配置和机器可读结果，再提供 GitHub Actions 示例，最后以薄封装方式评估 Gradle 插件；GUI 批量加固独立推进 | 待排期；不早于 CLI 自动化契约冻结 |
| 工具链轻量化 | 缩短加固链路并减少不必要的外部进程和整包重建 | 逐步接管 ZIP 条目重写、二进制 Manifest 定点修改、DEX/Native 注入与对齐；不支持的输入继续回退 apktool，签名仍优先复用 apksigner | 待专项原型验证，不以“完全去除 Java”为版本目标 |
| 可选资源保护 | 为离线模型、数据库、音视频等高价值文件提供显式保护能力 | 用户选择资源清单、认证加密载荷、应用侧读取接口、缓存与流式解密、体积和性能评估 | 需求驱动调研，不进入默认加固流程 |
| 后续保护增强 | 让提取结构载体后不能直接得到完整业务实现，同时避免引入类似恶意加载器的高风险行为 | DEX 方法清点、离线抽取、认证分层载荷和运行时路径评审已经完成；ART 私有 Hook、自修改代码和可写可执行内存不进入正式方案 | 当前约束下已停止产品接入；保留研究证据，不预占版本号，不进入 GUI |
...
|--------|----------|------------------|------------------|
| `1.3.0` 稳定版维护 | 并行观察 | 汇总反馈、复现真实缺陷、只发布必要修复 | 不阻塞后续版本开发 |
...
| Root 策略与内存 DEX 实验 | `alpha.2` 已发布 | GUI、核心、Manifest、双资源与兼容/严格策略链路已接通；普通真机和 LineageOS ADB Root 已完成双向回归。原加载器内存注入实验已止损，不合入运行代码 | 已进入 Beta 功能冻结；替换 ClassLoader 方案留待后续独立评估 |
...
API 31 以上内存 DEX 的正式接入、资源协议、状态存储、迁移回滚和停止条件见 [内存 DEX 正式接入设计](../design/memory-dex-production.md)。实施拆为候选资源契约、正式运行时最小接入和完整生产门禁三个完整阶段；每阶段独立提交和验证，不把探针实现直接复制进正式 Stub。

AAB 单 base 模块实验已经验证 DEX 尾部载荷、DEXB v5、ABI 配置 APK、多 DEX 和覆盖更新的本地与真机基础链路。由于尚未经过 Google Play 内部测试轨道和 Play 应用签名验证，当前正式版本仍只支持 APK。实验结论见 [AAB 加固可行性结论](../design/aab-feasibility.md)，正式能力独立安排在 `1.5.0`，不提前进入 `1.3.0` 或 `1.4.0`。

自动化和工具链轻量化采用渐进路线：先让现有核心能力具备稳定的非交互入口，再围绕真实瓶颈替换局部环节，不以一次性重写 apktool、Android 资源表或 APK 签名实现为目标。具体阶段、回退策略和验收门槛见 [工具链轻量化与自动化集成研究](../design/toolchain-modernization.md)。

资源保护与 DEX 保护是两条独立能力线。资源保护只面向用户明确选择的文件，通过应用侧接口读取；首轮不通过 Hook 透明拦截 `AssetManager`，也不承诺无需业务改造。具体边界见 [可选资源保护研究](../design/asset-protection.md)。
...
3. `1.3.0-alpha.2` 已完成兼容/严格 Root 环境策略、GUI 任务级选项、普通真机与 ADB Root 双向回归；API 29 以上内存 DEX 实验未达生产门槛并已关闭，不阻塞后续阶段。
...
6. 从 `1.4.0-alpha.1` 开始把 API 31 以上内存 DEX 作为生产候选，经 `1.4.0-beta.1` 扩大验证和 `1.4.0-rc.1` 发布冻结后，再决定发布 `1.4.0` 正式版。
...
|----|------|
| **优先级** | 中 |
| **状态** | 已否决（离线研究与运行时路径评审已完成） |
| **当前涉及模块** | `shield-core` 私有 DEX 研究模块与测试；暂不涉及 Stub、`DexInjector`、CLI 和 GUI |

**方向**：完整 DEX 加密只能阻止静态直接提取；运行时形成完整明文后仍可能被分析。下一阶段先在宿主端清点 DEX 方法、抽离普通方法指令、生成占位载体并完成离线往返恢复。该原型只证明处理器正确性和静态保护收益，不生成用户 APK，也不设计正式加密协议。离线证据通过后才评估认证载荷和运行时恢复，不把单纯破坏 DEX header 作为有效保护。

**当前结果**：`shield-core` 已建立仅测试构建启用的私有研究模块，完成 DEX header、字符串、类型、原型、方法、类数据与 `code_item` 的只读清点；合成 DEX 的稳定性和错误路径测试通过，仓库 Android 冒烟工程实际编译的 DEX 已完成本地验证。没有新增依赖、公共接口或正式产物能力。

离线转换已经从普通候选方法抽取指令并生成类型正确的占位实现，恢复前校验载体摘要、边界和占位内容。仓库 Android 冒烟工程的 8304 字节真实 DEX 抽取出 19 个候选方法，占位载体与恢复产物均通过 Android `dexdump`，恢复产物与原始 DEX 的 SHA-256 完全一致；这一阶段只形成结构结论，后续再补充实际运行语义。

首批 Java 可执行语义已补齐：专用业务样本覆盖 int、long、对象、数组、boolean、double、void、switch、try/catch 和 synchronized，共抽取 10 个候选方法。普通构建与启用 R8 的发布型研究构建均生成原始、占位和恢复 APK，使用同一测试签名在 API 35 OnePlus 与 API 33 Classin 真机完成安装和冷启动；原始与恢复输出完全一致，占位输出为预期默认值且应用未崩溃。R8 样本通过保留业务类身份验证反射调用，尚未覆盖任意混淆类的映射定位。

Kotlin 样本进一步覆盖默认参数桥接、Lambda、数据类合成方法和真实挂起恢复的协程状态机。R8 产物共抽取 24 个 Kotlin 候选方法，两台真机上原始与恢复语义一致、占位返回默认值；只抽取并实际触发 `invokeSuspend` 时，占位结果明确退化而恢复结果重新正确，证明协程状态机经过真实执行。

多 DEX 样本把观察入口和主包业务方法放在 `classes.dex`，次包业务方法放在 `classes2.dex`，通过主到次、次到主的反射调用形成双向链路。两个 DEX 分别抽取 1 个候选方法，并在 API 35 OnePlus 与 API 33 Classin 真机验证原始、仅主包占位、仅次包占位、双包占位和双包恢复五种形态；语义退化位置与选择范围一致，双包恢复与原始完全相同，不提前接入正式多 DEX 载荷协议。

ARouter 样本使用 1.5.2 注解处理器生成真实 Root、Group 和 Providers 路由表，并在 R8 研究构建中通过运行期 DEX 扫描初始化。两台真机分别验证目标业务方法和 `ARouter$$Group$$research.loadInto()`：业务占位时路由仍能解析而业务结果退化，路由表占位时路径明确不可匹配，恢复后均回到原始结果。每种形态均清除应用数据，排除路由缓存干扰。

JNI 样本使用 NDK r29 构建独立 ARM64 测试库，通过 `JNI_OnLoad` 和 `RegisterNatives` 动态注册，形成 Java 包装方法 → Native → Java 回调链。两台真机分别验证包装类与回调类：包装占位时不再进入 Native，回调占位时 Native 仍执行并得到空回调结果，恢复后重新得到完整结果；Native 声明本身保持跳过。

静态收益评估已覆盖仓库双 DEX 样本和一个经授权用于本地回归的 329 MB、12 DEX 真实 APK。仓库样本可抽离 67.59% 的声明方法和 85.87% 的指令字节；真实样本可抽离 76.14% 的声明方法和 86.21% 的指令字节。jadx 对比确认载体仍暴露类型、字段、签名、注解、字符串和包结构，但候选方法的业务实现已不可从方法体直接取得。当前异常表残留造成的反编译错误不计入安全收益，正式载体必须保持结构自洽。

继续/停止评审结论为继续离线研究：跳过比例尚未抵消保护收益，且被抽离指令不能通过修复少量 DEX 头字段机械恢复。下一步只设计认证的整组载荷清单、稳定 DEX 身份以及篡改和错序拒绝测试；未找到低风险运行时恢复路径前，不接入 Stub、APK、CLI 或 GUI。

认证整组清单已经完成测试私有实现：按稳定顺序绑定 DEX 数量、唯一身份、原始摘要、载体摘要和覆盖全部方法记录的代码载荷摘要，并使用现有 ChaCha20-Poly1305 完成研究用认证封装。构建前和恢复后都会核对原始 DEX，错误密钥、密文/载体/载荷篡改、相同载体下交换载荷、错序、缺失、新增及重复身份均失败关闭。该实现不冻结正式格式、密钥派生和 nonce 策略，也没有把代码载荷本身序列化或加密。

随后仅在测试私有边界内实施“清单 + 加密代码载荷”的可逆封装及解析预算，没有修改 DEXB v5 和正式运行时。

加密代码载荷离线封装已经完成：认证整组清单和每个 DEX 的方法代码记录被规范编码并整体认证加密，解封输入只需有序身份与占位载体。双 DEX 已完成逐字节恢复，并拒绝错误密钥、密文篡改、载体错序和缺失；解析器对 magic、版本、截断、尾随数据、DEX/方法数量、单方法长度和 256 MiB 总明文预算均失败关闭。当前固定 nonce 只服务确定性测试，正式密钥派生、随机 nonce 和二进制格式尚未冻结。

随后使用仓库样本和已授权真实 12 DEX 样本测量代码载荷封装体积、编码/解封耗时与峰值内存，以决定整体密文是否需要改为逐 DEX 独立认证分片；该过程没有修改正式运行时。

真实资源评估已经完成。12 个 DEX 共 118708156 字节、可抽取指令 30111218 字节；移除密文中重复的占位指令后，认证密文从 68347120 字节降至约 3621 万字节，占原 DEX 总量 30.50%。逐 DEX 顺序处理只比整组增加 440 字节，抽取、封装和解封总耗时基本相同，缓存后的系统峰值约 144 MiB，相对整组约 803 MiB 降低约 82%。

资源评审决定停止整体大密文方向，随后验证“小型认证总索引 + 每个 DEX 独立认证密文”：总索引绑定顺序、身份、载体摘要、密文摘要和 nonce，单 DEX 可以顺序解封并释放；该过程仍未接入 Stub、APK、CLI 或 GUI。

分层认证索引已经完成测试私有闭环。96 位包级随机 nonce 基值配合索引/分片计数器生成包内唯一 nonce；总索引认证 DEX 顺序、身份、载体摘要、分片 nonce 和密文摘要。解封只有在整组索引通过后才逐 DEX 恢复并通过回调立即交付。缺失、分片或载体错序、密文/索引篡改、跨组替换和重复 nonce 均失败关闭。

已授权真实 12 DEX 样本也已完成分层闭环，全部分片按稳定顺序逐字节恢复。认证总索引密文仅 1110 字节，逐 DEX 分片密文合计 36205686 字节，总索引额外成本约 0.003%。

离线格式与资源方向至此收口。运行时路径评审确认：公开 `InMemoryDexClassLoader` 只能接收完整 DEX，`AppComponentFactory` 只能解决类加载器选择和初始化时序；顺序解封可以降低临时峰值，但最终仍需形成完整 DEX，无法绕过已观察到的长期内存成本。文件重建会恢复完整明文文件，多类加载器会破坏跨 DEX 类身份，ART 私有回填和 Hook 又违反安全边界。因此本方向停止产品接入，保留测试私有研究成果，不修改正式 Stub、DEXB v5、APK、CLI 或 GUI。

**安全边界**：借鉴 dpt-shell 的方法代码分离思想，不移植其 Hook 实现。ART 私有函数和系统函数 Hook、可写可执行代码页、自修改 Native 代码、运行时解密 Native 代码段均禁止进入正式方案。本方向不新增或扩张反 Frida 等对抗行为；出现相对同签名基线的稳定安全软件检出增量时必须定位最小触发行为，无法删除风险行为则停止运行时方案。

**进入运行时研究的条件**：离线往返、Java/Kotlin/R8 等兼容样本与静态提取实验全部通过；结构载体不能通过机械修复直接恢复完整业务代码；能够提出不依赖 ART 私有 Hook、可写可执行内存和自修改代码的恢复方案。完整内存 DEX 不必先正式发布，但其长期内存成本不能被结构分离绕过。详细阶段、参考实现审查、安全软件门禁和停止条件见 [DEX 结构与方法代码分离研究](../design/dex-code-separation.md)。
...
把更多壳逻辑加密保存并在运行时加载只保留为 API 31 以上独立实验：初始 DEX 仍保留最小 Application 引导，二阶段代码不得成为业务类的 defining loader。每个 ABI 的 SO 重复内嵌密文不作为默认方案，运行期动态生成 DEX 明确不采用。主路线可以独立排期；二阶段实验必须等待高版本内存 DEX 路径稳定，不阻塞正式版本。详细方案与停止条件见 [Stub DEX 最小化与分阶段加载可选演进](../design/stub-dex-minimization.md)。
