...
| Root 策略与内存 DEX 实验 | Root 已完成，内存实验已止损 | Root 策略链路已完成；API 29+ 原加载器内存注入受 ART 多加载器限制和非 SDK 接口约束，不合入运行代码，正式资源保持关闭 | Root 进入 `alpha.2`；内存实验结论不阻塞后续 Beta，替换 ClassLoader 方案留待后续独立评估 |
