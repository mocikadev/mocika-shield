...
| Native 库名去品牌化 | 进行中 | 先完成元数据协议、名称生成与 Rust 测试，再实现 Stub DEX 等长替换和端到端验证 | 与认证缓存共同进入 `alpha.1`，但保持独立提交和回归结论 |
| Stub DEX 基线治理 | 可并行准备 | 建立类、方法、字符串和体积指标，识别无消费诊断入口 | 仅做度量与静态审计，不同时改运行时行为 |
| 认证缓存与资源能力 | 下一项 | Native 库别名合并后实现摘要、原子重建与协议握手 | 与资源打包、Stub 边界有交叉，按技术顺序串行实施；完成后发布 `alpha.1` |
