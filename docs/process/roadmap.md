...
| `1.4.0` 替换 ClassLoader 原型 | 基础兼容矩阵已完成 | API 28～30 已通过认证文件路径；API 31 最低内存边界、API 33 Classin 与 API 35 OnePlus 两家厂商真机已通过内存双 DEX、五类回调和双进程独立认证状态 | Android 10、11 的启动前 Keystore 上下文不成立，内存边界为 API 31+；下一步自动化状态异常、双向迁移和性能门禁，完整门槛通过前不进入 GUI |
