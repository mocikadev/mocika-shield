...
| `1.4.0` 替换 ClassLoader 原型 | 进行中 | 框架版本矩阵、正式 DEXB v5、原组件工厂、ARouter、Instrumentation、split 安装和内存/文件双向覆盖迁移已通过首轮真机验证；诊断确认临时载荷对象可回收，定长 ZIP 读取减少约 3.5 MB 瞬时峰值，同条件性能基线未观察到冷启动回退 | 代码 split 仍为明文，崩溃自动回退尚未实现，稳定 PSS 仍比文件路径高约 5.4 MB；下一步优先设计认证回退状态机，Native 直接缓冲区作为独立性能研究，完整门槛通过前不进入正式资源 |
