...
| `1.4.0` 替换 ClassLoader 原型 | 正式接入设计完成 | 框架版本矩阵、正式 DEXB v5、原组件工厂、ARouter、Instrumentation、split 安装、内存/文件双向迁移、认证崩溃回退、状态篡改、包装密钥异常和双进程并发启动均已通过首轮真机验证；正式候选采用独立资源、显式选择、默认关闭 | 下一步先实现候选资源契约与构建隔离，再接入正式运行时；代码 split 仍为明文，稳定 PSS 仍比文件路径高约 5.4 MB，完整门槛通过前不进入 GUI |

用户诊断中的低风险静态预检可以按真实反馈独立插入，不必等待整条安全主线完成；但不得与高风险壳加载改动混在同一提交或候选版本中。

API 29 以上内存 DEX 的正式接入、资源协议、状态存储、迁移回滚和停止条件见 [内存 DEX 正式接入设计](../design/memory-dex-production.md)。实施拆为候选资源契约、正式运行时最小接入和完整生产门禁三个完整阶段；每阶段独立提交和验证，不把探针实现直接复制进正式 Stub。
