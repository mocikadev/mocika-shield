...
**后续安全演进**：当前检查只在缓存未命中、进入 Native 解密时执行，缓存命中会绕过启动期检测；缓存本身也缺少签名锚定的完整性验证。后续按以下顺序推进：

1. 在 `1.2.7-rc.5` 将环境检查调整为每次启动必经，同时保留解密入口纵深检查。
2. 在 `1.3.0-alpha.1` 引入由最终 APK 签名保护的缓存根摘要、原子缓存重建和资源能力协议。
3. 在 `1.3.0-beta.1` 接入兼容/严格 Root 环境策略和 GUI 任务级选项。
4. `1.3.0-rc.1` 只做完整安全回归与缺陷修复，验证后再发布稳定版。
5. Android 8.0 以上内存 DEX 作为 `1.4.0-alpha.1` 独立实验，不阻塞前述安全能力。
