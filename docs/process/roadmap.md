...
| `1.4.0` 替换 ClassLoader 原型 | 候选资源契约完成 | 框架版本矩阵和认证回退已通过首轮真机验证；核心已支持协议 3、显式候选授权、原组件工厂保存与候选 Manifest 转换，自动资源发现仍只接受标准资源 | 下一步接入真实壳组件工厂、稳定代理和正式运行时后再生成 `resources-memory.zip`；代码 split 仍为明文，稳定 PSS 仍比文件路径高约 5.4 MB，完整门槛通过前不进入 GUI |
