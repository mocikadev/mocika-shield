...
| `1.4.0` 替换 ClassLoader 原型 | 大 DEX 内存门禁未通过 | 状态、迁移和分段剖析已完成；113 MB DEX 的临时 Java 数组可回收，但 API 35 的直接缓冲区和 ART 匿名映射使稳定 PSS 增加约 71 MB，加载器建立单轮最高 1.77 秒 | Android 10、11 固定使用文件路径；先做 JNI 直接缓冲区原型降低启动峰值，再判断 ART 常驻成本是否可接受；结论明确前不进入 GUI 或 Alpha 集成版本 |
...
**前置条件**：`1.3.0` 运行时安全主线稳定，且 `1.4.0` API 31 以上完整 DEX 内存加载达到正式生产门槛。首个原型只覆盖 API 31 以上、单 DEX 和普通 Java 方法，不进入 GUI 或正式资源。
