...
| 阶段 | 目标 | 重点任务 | 版本建议 |
|------|------|----------|----------|
| 稳定性巩固 | 把已经验证过的主流程固化，减少回归风险 | 核心链路回归测试、CI 产物自检、错误提示收敛、Release 检查补强 | `1.2.x` |
| 用户诊断能力 | 让普通用户遇到问题时能看懂原因并提供有效反馈 | Java/签名/alias/密码/APK 非法/16 KB 对齐等错误分类；复制诊断信息；本地加固报告 | `1.3.x` |
| 兼容性预检 | 在真正加固前提前暴露 APK 风险 | minSdk/targetSdk、ABI、split APK、多 dex、已有壳、签名方案、Google Play 16 KB 风险扫描 | `1.3.x` / `1.4.x` |
| 发布格式拓展 | 验证 Google Play 新应用所需的 AAB 加固路径 | DEX 尾部载荷保真、Play 应用签名绑定、bundletool 与模块化交付兼容性 | 实验性 |
| 自动化与批处理 | 支持更高频、重复性使用场景 | GUI 批量加固、CLI 配置文件、CI 集成模式、可复用任务模板 | `1.4.x` |
| 保护能力增强 | 在稳定基础上继续提升逆向成本 | 更细粒度反调试、运行时完整性检测、壳特征继续降低、可配置保护级别 | `1.5.x+` |
| 供应链与发布成熟度 | 提升开源用户信任与正式分发体验 | 安装包代码签名、公证、SBOM、release attestation、依赖审计自动化 | 持续推进 |

### 迭代原则

- `main` 保持可发布，复杂功能使用临时 `feat/*` 或 `fix/*` 分支开发，技术可行性实验使用 `spike/*`，合并或结束后删除
...
| **优先级** | 高 |
| **状态** | 进行中 |
| **涉及模块** | `experiments/aab-feasibility`、`docs/design/aab-feasibility.md` |

当前只验证单 base 模块，不接入正式 APK 加固接口。本地 bundletool 尾部保真、真实 DEXB v5 真机加载、ABI 配置分包、双 DEX 和覆盖更新均已通过；下一门禁是 Play 内部测试轨道和 Play 应用签名证书绑定。

若 Google Play 会清理尾部数据或拒绝该 AAB，则停止复用现有 DEXB v5 容器，单独评估新的 AAB 载荷方式。动态功能模块、Asset Pack 和按需交付不属于首轮验证范围。完整实验设计与结论记录见 [AAB 加固可行性验证](../design/aab-feasibility.md)。
