...
| `1.4.0` 替换 ClassLoader 原型 | 状态与迁移门禁已完成 | API 28～30 文件路径和 API 31+ 内存边界已通过；API 33 Classin 与 API 35 OnePlus 已通过双向覆盖、崩溃回退、文件连续失败、进程隔离及三类认证异常 | Android 10、11 固定使用文件路径；下一步量化标准/候选冷启动和 PSS，性能门槛通过前不进入 GUI |
