...
| Root 策略与内存 DEX 实验 | `alpha.2` 发布候选完成 | GUI、核心、Manifest、双资源与兼容/严格策略链路已接通；普通真机和 LineageOS ADB Root 已完成双向回归。API 29+ 原加载器内存注入实验已止损，不合入运行代码 | 发布 `alpha.2` 后进入 Beta 功能冻结；替换 ClassLoader 方案留待后续独立评估 |
