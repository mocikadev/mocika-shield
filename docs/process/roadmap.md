...
| `1.4.0` 替换 ClassLoader 原型 | 正式运行时最小链路进行中 | 协议 3、真实壳组件工厂、稳定代理、正式 DEXB v5 内存解密和候选资源已接入；API 35 真机通过双 DEX 冷启动，私有目录未生成明文 DEX | 下一步实现认证崩溃回退，并补 API 29、API 28 以下文件回退和原组件工厂样本；代码 split 仍为明文，完整门槛通过前不进入 GUI |
