...
| 缓存安全与环境策略 | 区分缓存完整性和明文保密性，提供可选择的运行环境策略 | Native 库名去品牌化、缓存摘要、原子重建、资源能力协议、API 29+ 内存 DEX 实验、Root 兼容/严格策略、Stub DEX 静态治理 | 已完成：`1.3.0` |
...
| `1.4.0` 替换 ClassLoader 原型 | 兼容矩阵进行中 | API 28、29 已通过认证文件路径、原组件工厂五类回调和双进程；API 35 真机已通过内存双 DEX、五类回调和双进程独立认证状态 | Android 10、11 的启动前 Keystore 上下文不成立，内存边界已调整为 API 31+；下一步补 API 30、API 31 最低边界和另一台不同厂商真机，完整门槛通过前不进入 GUI |
