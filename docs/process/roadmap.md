...
| `1.4.0` 替换 ClassLoader 原型 | 进行中 | API 29、API 35 16 KB、API 36 模拟器与 API 35 真机上的框架矩阵已通过。API 35 真机进一步通过正式 Stub Native、核心生成的 DEXB v5 双 DEX、同签名解密成功与异签名失败关闭验证；稳定代理支持 Context 后初始化、重复初始化、并发加载和 API 29 多工厂实例 | 下一步验证原工厂元数据、ARouter 和 Android 9 系统共享库，再覆盖 split、Instrumentation、覆盖安装和回退；完整门槛通过前不进入正式资源 |
