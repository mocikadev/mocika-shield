...
| `1.4.0` 替换 ClassLoader 原型 | 性能门禁待优化 | 状态与迁移门禁已完成；小型双 DEX 显示冷启动回退 8.4%～12.2%；329 MB、12 DEX 真实样本已通过两台真机功能验证，但日常冷启动中 API 33 回退 7.7%，API 35 内存 PSS 波动明显 | Android 10、11 固定使用文件路径；下一步同时做启动分段剖析和大 DEX 直接缓冲区、GC、内存映射分析，解释波动前不进入 GUI 或 Alpha 集成版本 |
