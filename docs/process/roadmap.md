...
| `1.4.0` 替换 ClassLoader 原型 | 启动峰值优化通过，自动预算进入候选验证 | JNI 直接缓冲区使 113 MB DEX 的进程高水位下降约 113 MB；协议 3 候选已按 API、总/可用内存、低内存标记、进程位数和 DEX 总量在解密前选择路径，并区分预算文件成功与崩溃粘性回退 | 补齐预算决策矩阵、Manifest 契约、端到端与真机校准；CPU 只作性能观测。标准和工控资源不变，结论明确前不进入 GUI 或 Alpha 集成版本 |
