...
| `1.4.0` 替换 ClassLoader 原型 | 启动峰值优化通过，长期内存门禁未通过 | JNI 直接缓冲区使 113 MB DEX 的进程高水位下降约 113 MB，API 33 启动改善 3.6%；但 API 35 稳定 PSS 和匿名映射未下降 | Android 10、11 固定使用文件路径；下一步确认 ART 常驻成本是否为固有限制，并制定 DEX 体积与设备内存适用边界；结论明确前不进入 GUI 或 Alpha 集成版本 |
