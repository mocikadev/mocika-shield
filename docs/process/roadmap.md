...
| 缓存安全与环境策略 | 区分缓存完整性和明文保密性，提供可选择的运行环境策略 | Native 库名去品牌化、缓存摘要、原子重建、资源能力协议、API 29+ 内存 DEX 实验、Root 兼容/严格策略 | `1.3.0-alpha.1` → `alpha.2` → `alpha.3` → `beta.1` → `rc.1` → `1.3.0` |
...
| Native 库名去品牌化 | 立即实施 | 先完成元数据协议、名称生成与 Rust 测试，再实现 Stub DEX 等长替换和端到端验证 | 当前主任务；范围清晰、设计和验收矩阵已具备 |
| Stub DEX 基线治理 | 可并行准备 | 建立类、方法、字符串和体积指标，识别无消费诊断入口 | 仅做度量与静态审计，不同时改运行时行为 |
| 认证缓存与资源能力 | 下一项 | Native 库别名合并后实现摘要、原子重建与协议握手 | 与资源打包、Stub 边界有交叉，按技术顺序串行实施 |
| 内存 DEX 与 Root 策略 | 暂不实施 | 保留实验夹具和验证条件，不提前接入正式 GUI | 分别等待资源协议和缓存路径稳定；不是等待下载数据 |
...
2. 在 `1.3.0-alpha.1` 独立交付 Native 库名去品牌化，先移除固定品牌文件名特征，不同时修改缓存、Root 策略或内存加载。
3. 在 `1.3.0-alpha.2` 引入由最终 APK 签名保护的缓存根摘要、原子缓存重建和资源能力协议；该阶段只解决完整性，不宣称防提取。
4. 在 `1.3.0-alpha.3` 实验 API 29 以上内存 DEX，默认关闭且不进入正式 GUI，以实际兼容和性能数据决定后续范围。
...
| native 库名 | 当前仍为固定 `mocikashield`；已规划按任务生成中性别名并联动改写 Stub DEX |

#### Native 库名去品牌化与按任务别名（待实现）

固定 `libmocikashield.so` 会直接暴露工具身份；改成另一个固定名称只会形成新指纹，纯十六进制随机名称又过于突兀。后续采用固定长度的中性自然别名：每次加固生成无冲突名称，联动等长修改 Stub DEX 的加载占位符，并把四 ABI 规范库映射到同一个任务别名。
