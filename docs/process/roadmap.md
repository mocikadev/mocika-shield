...
| `1.4.0` 替换 ClassLoader 原型 | 进行中 | 框架版本矩阵、正式 DEXB v5、原组件工厂、ARouter、Instrumentation、split 安装和内存/文件双向覆盖迁移已通过首轮真机验证；已建立五次冷启动与即时内存中位数基线 | 代码 split 仍为明文，崩溃自动回退尚未实现，内存原型即时 PSS 明显偏高；先收敛载荷长期持有和回退状态机，再扩大厂商矩阵，完整门槛通过前不进入正式资源 |
