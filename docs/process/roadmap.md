...
| `1.4.0` 替换 ClassLoader 原型 | 进行中 | 框架版本矩阵、正式 DEXB v5、原组件工厂、ARouter、Instrumentation、split 安装、内存/文件双向迁移、认证崩溃回退和状态篡改拒绝均已通过首轮真机验证；诊断确认临时载荷对象可回收，定长 ZIP 读取减少约 3.5 MB 瞬时峰值 | 代码 split 仍为明文，稳定 PSS 仍比文件路径高约 5.4 MB；下一步验证 Keystore 丢失和多进程竞争，再扩大厂商设备矩阵，完整门槛通过前不进入正式资源 |
