...
| 缓存安全与环境策略 | 区分缓存完整性和明文保密性，提供可选择的运行环境策略 | Native 库名去品牌化、缓存摘要、原子重建、资源能力协议、API 29+ 内存 DEX 实验、Root 兼容/严格策略 | `1.3.0-alpha.1` → `alpha.2` → `beta.1` → `rc.1` → `1.3.0` |
...
| 后续保护增强 | 让提取单一载体或快照后不能直接得到完整业务 DEX | DEX 结构与方法代码分离、内存重建、分片恢复可行性；壳特征持续治理 | `1.4.0` 内存 DEX 正式版后专项调研，不预占版本号 |
