...
| `1.4.0` 替换 ClassLoader 原型 | 性能门禁待优化 | 状态与迁移门禁已完成；双顺序各 7 轮显示 OnePlus 冷启动 +8.4%、Classin +12.2%，整理后双进程 PSS 分别 +1.8 MB、+0.5 MB | Android 10、11 固定使用文件路径；Classin 超出 10% 冷启动参考线，下一步做启动阶段分段剖析，门禁通过前不进入 GUI |
