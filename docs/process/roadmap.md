...
| `1.4.0` 替换 ClassLoader 原型 | 进行中 | 框架版本矩阵和正式 DEXB v5 签名绑定链路已通过。API 35 真机进一步完成原组件工厂元数据的自定义、默认与递归失败关闭验证；真实 AndroidX/ARouter 样本通过覆盖升级、清除数据和全新安装。API 28 文件加载回退通过清除数据与覆盖升级 | 下一步覆盖 split、Instrumentation、内存/文件版本迁移回退和性能基线；完整门槛通过前不进入正式资源 |
