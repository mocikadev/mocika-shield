...
| `1.4.0` 替换 ClassLoader 原型 | 兼容矩阵进行中 | API 28～30 已通过认证文件路径；API 31 最低内存边界和 API 35 OnePlus 真机已通过内存双 DEX、五类回调和双进程独立认证状态 | Android 10、11 的启动前 Keystore 上下文不成立，内存边界为 API 31+；下一步补另一台不同厂商真机，完整门槛通过前不进入 GUI |
