...
| `1.4.0` 替换 ClassLoader 原型 | 正式运行时最小链路完成 | 协议 3、真实壳组件工厂、稳定代理、正式 DEXB v5 内存解密和认证崩溃回退已接入；API 35 真机通过双 DEX、真实崩溃后粘性文件回退及状态篡改失败关闭 | 下一步补 API 29、API 28 以下文件路径、原组件工厂和多进程正式样本；代码 split 仍为明文，完整门槛通过前不进入 GUI |
