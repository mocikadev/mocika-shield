...
- 加固失败时可复制脱敏诊断摘要；摘要不包含 APK/输出路径、证书、密码、包名、业务类名或原始错误文本
- 预检出现风险或阻断时可复制脱敏诊断摘要；摘要只包含稳定风险代码、运行策略和当前阶段
...
- 每个应用进程必须拥有独立的回退记录和认证密钥；主、远程进程并发首次启动不得互相覆盖状态或触发错误回退
- 包装密钥密文损坏或对应 Keystore 条目丢失时不得静默生成新密钥，必须在解密和业务类定义前失败关闭
...
### 2026-07-31：单 APK 预检与诊断回归校准

| 项 | 结果 |
|----|------|
| 自动检查 | `cargo clippy -p mocika-shield --all-targets -- -D warnings`、GUI 后端 42 项测试、前端 lint 与生产构建通过；二进制 Manifest 解析覆盖 SDK、split、`extractNativeLibs` 与 HTTP 兼容信号 |
| Android 15 真机 | OnePlus 5 / LineageOS / API 35 / `arm64-v8a`：项目双 DEX 测试夹具完成未加固基线、同签名覆盖安装加固包首次启动和缓存命中二次启动；`Application`、`Activity`、Provider、远程 Service 与第二 DEX 标记均正常 |
| Android 13 工控设备 | ClassIn Home / API 33 / `arm64-v8a`：同一双 DEX 测试夹具完成未加固基线、同签名覆盖安装加固包首次启动和缓存命中二次启动，端到端脚本通过 |
| 真实样本静态检查 | ClassIn 原包为已签名、未加固、`arm64-v8a` + `armeabi-v7a`；对应加固样本被正确识别为已加固。另一个真实原包/加固包对均为已签名且证书 SHA-256 一致 |
| 结论边界 | 本轮真实样本只做只读结构与签名检查，不把它记作真实业务 APK 的安装启动回归；GUI 预检展示和诊断摘要的人工交互仍应使用更新后的桌面应用单独确认 |

**阶段判断**：单 APK 的核心预检、结构化诊断和基础真机链路没有发现回归。后续以真实样本的 GUI 人工确认、16 KB 及 Android 4.4 历史矩阵复核为准，不恢复批量加固范围。

### 2026-07-31：Stub DEX 指标基线校准

**阶段判断**：使用当前标准资源重新生成的基线为 29,376 字节、19 个类、287 个方法、51 个字段和 475 个字符串。旧阈值仍对应功能收敛前的 17 KB 产物，已改为仅保留约 10%～18% 增量空间的上限；完整 CI 继续对意外膨胀失败关闭。
