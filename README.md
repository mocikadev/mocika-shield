...
| 场景 | 要求 |
|------|------|
| 使用 CLI（已构建程序） | Java 8+（需完整 JDK，`java` / `keytool` 可用） |
| 使用发布包（GUI） | Linux / macOS / Windows，Java 8+（加固、签名、Alias 识别需要完整 JDK） |
...
Beta.6 起，加固与签名页另有独立的“分享使用情况”选择：新应用默认选中，同一包名取消后会记住。成功时分享应用名称、包名、版本及操作信息，仅维护者可见、保留 180 天；不上传 APK 或设备标识。此选择不受匿名统计开关控制，取消不删除已接收记录。

具体数据范围及三种通道的控制方式见[数据与隐私说明](docs/ops/telemetry.md)。
